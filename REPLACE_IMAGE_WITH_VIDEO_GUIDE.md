...
This guide provides step-by-step instructions on how to replace a static image (like `/Composite-sliding-patio-door.png`) on your homepage with a video.
...
3.  **Location:**
    *   If it doesn't exist, create a `public/` directory in your project root.
...
Locate the `<img>` tag you wish to replace. In your case, it's the one that would reference `/Composite-sliding-patio-door.png`.

*Note: In the current project state, this corresponds to the placeholder image at line 85 within the `.waffle-card` container.*
...
Ensure the class used for the element (e.g., `.waffle-image`) is configured to handle the video correctly. In `styles/components.css` (line 61), it should look like this:

```css
.waffle-image {
    width: 100%;
    height: 100%;
    object-fit: cover; /* This ensures the video fills its container without distortion */
}
```
